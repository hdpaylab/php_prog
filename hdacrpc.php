<?php
	...
	//	echo "===$url $user=$password ===<PRE>"; print_r($payload); echo '</PRE>';
	...
	function hdac($method) // other params read from func_get_args()
	{
		global $hdac_chain;
		
		$args=func_get_args();
		
		return json_rpc_send($hdac_chain['rpchost'], $hdac_chain['rpcport'], $hdac_chain['rpcuser'],
			$hdac_chain['rpcpassword'], $method, array_slice($args, 1));
	}
	...
	function hdac_getinfo()
	{
		global $hdac_getinfo;
		
		if (!is_array($hdac_getinfo))
			no_displayed_error_result($hdac_getinfo, hdac('getinfo'));
		
		return $hdac_getinfo;
	}
	
	function hdac_labels()
	{
		global $hdac_labels;
		
		if (!is_array($hdac_labels)) {
			if (no_displayed_error_result($items, hdac('liststreampublishers', 'root', '*', true, 10000))) {
				$hdac_labels=array();
				foreach ($items as $item)
					$hdac_labels[$item['publisher']]=pack('H*', $item['last']['data']);
			}
		}
	...
	function hdac_max_data_size()
	{
		global $hdac_max_data_size;
		
		if (!isset($hdac_max_data_size))
			if (no_displayed_error_result($params, hdac('getblockchainparams')))
				$hdac_max_data_size=min(
					$params['maximum-block-size']-80-320,
					$params['max-std-tx-size']-320,
					$params['max-std-op-return-size']
				);
		
		return $hdac_max_data_size;
	}	
